...
#check if two nodes are verically above and below
	...
	if(go_down and row<len(contours)-1 ):
		...
		for next_row in range(row+1,min(row+4,len(contours))):
			...
			for i in range(0,len(contours[next_row])):
				...
				if(x_overlapping(x1,x2,y1,y2,a1,a2,b1,b2) ):
					if not Nodes[contours[next_row][i]].is_key:
						...
						dfs(img,row,column+1,contours, Nodes, go_down,False,parent,root)
						flag=1
					else: break
